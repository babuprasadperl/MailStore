...
sub BUILD {
  my $self = shift;
  $self->init();
  logger()->info('Initalization complete: Object is ready');
  return $self;
}
...
sub init {
    ...
    try {
        ...
        logger()->info('Manager object is created');
        #
        # Step - 2: Check if source folder is reachable
        #
        unless($self->check_source_folder()) {
            logger()->info($self->{'source'}.' folder cannot be reached');
            return;
        }
        #
        ...
        logger()->info('Mail folder '.$self->{'source'}.' contains '. scalar @messages.' messages');
        #
...
sub handle_message {
    ...
    try {
        ...
        unless ($store_status) {
            $self->{log}->error("Could not create storage for id : $messageId");
            return;
        }
        #
        ...
        print "SERIlAIZE status = $serialize_status\n";
        unless ($serialize_status) {
            $self->{log}->error("Could not serialize object : $messageId");
            return;
        }
        ...
        if($msg->isMultipart) {
            ...
            #
            print "Number of attachments: ", scalar $body->parts('ACTIVE')."\n\n";
            my $save_count = 0;
        ...
        print "in error handling message : $!\n";
...
sub update_manifest{
    ...
    print "finished updating manifest\n";
    return 1;
}
